...
#Dish
def create_dishes(data:UserDTO.dish, db:Session):
    UserDTO=dish(id=data.id, dishc=data.dishc,price=data.price, submenu_id=data.submenu_id,submenudishs=data.submenudishs)
    try:
        db.add(UserDTO)
        db.commit()
        db.refresh(UserDTO)
...
# def update_dishes(data:user.dish, db:Session, id:int):
